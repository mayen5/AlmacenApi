...
public class TipoEmpaqueRestController {
    ...
    public ResponseEntity<?> show(@PathVariable Long id){
        ...
        TipoEmpaque tipoEmpaqueEncontrado = this.tipoEmpaqueService.findById(id);
        if (tipoEmpaqueEncontrado == null){
            response.put("mensaje", "Tipo de Empaque no encontrado");
            return  new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
        }

        return  new ResponseEntity<TipoEmpaque>(tipoEmpaqueEncontrado, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> create (@Valid @RequestBody TipoEmpaque elemento, BindingResult result){
        ...
        response.put("mensaje","El tipo de Empaque ha sido creado con éxito");
